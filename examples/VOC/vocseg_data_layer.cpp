...
#include "caffe/layers/vocseg_data_layer.hpp"
...
VocSegDataLayer<Dtype>::VocSegDataLayer(const LayerParameter& param)
...
template <typename Dtype>
VocSegDataLayer<Dtype>::~VocSegDataLayer() {
  this->StopInternalThread();
}

template <typename Dtype>
void VocSegDataLayer<Dtype>::loadDatums(const std::string& filePath){
	datums_.clear();
...
template <typename Dtype>
void VocSegDataLayer<Dtype>::DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
...
template<typename Dtype>
void VocSegDataLayer<Dtype>::load_batch(Batch<Dtype>* batch) {
...
template<typename Dtype>
void VocSegDataLayer<Dtype>::TransformDataLabel(Blob<Dtype>* data_blob, Blob<Dtype>* label_blob){
	VocSegDatum oriDatum;
...
//	cv::Mat label = segLabelImg==datum.boxes[ranObjInd].instanceID;
	...
	CHECK_GT(dominantInstanceID,0)<<oriDatum.imgPath<<":"<<x1<<","<<y1<<","<<x2<<","<<y2;
...
template<typename Dtype>
VocSegDatum VocSegDataLayer<Dtype>::flipVocSegDatum(const VocSegDatum& datum, int mode){
	VocSegDatum flippedDatum = datum;
...
template<typename Dtype>
void VocSegDataLayer<Dtype>::randomAssignDetected(VocSegDatum& datum,const cv::Mat& labelImg,cv::Mat& memImg){
	...
	std::vector<VocSegBox> unDetectedBoxes;
...
template<typename Dtype>
int VocSegDataLayer<Dtype>::getDominantInstanceID(const cv::Mat& labelImg){
	int fre[300] = {};
	for(int y=0;y<labelImg.rows;y++){
		for(int x=0;x<labelImg.cols;x++){
			int aLabel = labelImg.at<uchar>(y,x);
			CHECK_LT(aLabel,300)<<"sorry I can't handle label bigger than 40000";
			if(aLabel>0 && aLabel<255){
				fre[aLabel]++;
			}
		}
	}
...
INSTANTIATE_CLASS(VocSegDataLayer);
REGISTER_LAYER_CLASS(VocSegData);
