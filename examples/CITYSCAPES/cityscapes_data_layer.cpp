...
#include "caffe/layers/cityscapes_data_layer.hpp"
...
CityscapesDataLayer<Dtype>::CityscapesDataLayer(const LayerParameter& param)
...
template <typename Dtype>
CityscapesDataLayer<Dtype>::~CityscapesDataLayer() {
  this->StopInternalThread();
}

template <typename Dtype>
void CityscapesDataLayer<Dtype>::loadDatums(const std::string& filePath){
	datums_.clear();
	...
	while(std::getline(rootFile,rootPath)){
		CityscapesDatum datum;
		datum.imgPath = rootPath;
		
		std::ifstream csvFile((rootPath+"_gtFine_box.left.csv").c_str());
		string boxes;
...
template <typename Dtype>
void CityscapesDataLayer<Dtype>::DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
...
template<typename Dtype>
void CityscapesDataLayer<Dtype>::load_batch(Batch<Dtype>* batch) {
...
template<typename Dtype>
void CityscapesDataLayer<Dtype>::TransformDataLabel(Blob<Dtype>* data_blob, Blob<Dtype>* label_blob){
	const int ranIdx = rand()%datums_.size();
	CityscapesDatum oriDatum = datums_[ranIdx];
	
	//data
	cv::Mat aImg = cv::imread(oriDatum.imgPath+"_leftImg8bit.left.png");
//	cv::resize(aImg,aImg,cv::Size(), 0.5, 0.5);
	cv::Mat labelImg = cv::imread(oriDatum.imgPath+"_gtFine_instanceIds.left.png",CV_LOAD_IMAGE_ANYCOLOR | CV_LOAD_IMAGE_ANYDEPTH);
...
//	cv::waitKey(5000);
	...
	CityscapesDatum datum = oriDatum;
...
//	const int flip = rand()%4;
	...
	switch(flip) {
		...
		case 1 : cv::flip(fin_img,flip_img,0); //by x-axis
				 datum = flipCityscapesDatum(datum,0);
				 break;
		case 2 : cv::flip(fin_img,flip_img,1); //by y-axis
				 datum = flipCityscapesDatum(datum,1);
				 break;
...
template<typename Dtype>
CityscapesDatum CityscapesDataLayer<Dtype>::flipCityscapesDatum(const CityscapesDatum& datum, int mode){
	CityscapesDatum flippedDatum = datum;
...
template<typename Dtype>
void CityscapesDataLayer<Dtype>::randomAssignDetected(CityscapesDatum& datum,const cv::Mat& labelImg,cv::Mat& memImg){
	...
	std::vector<CityscapesBox> unDetectedBoxes;
...
INSTANTIATE_CLASS(CityscapesDataLayer);
REGISTER_LAYER_CLASS(CityscapesData);
