...
#include "caffe/layers/cvpppseg_data_layer.hpp"
...
CvpppSegDataLayer<Dtype>::CvpppSegDataLayer(const LayerParameter& param)
...
template <typename Dtype>
CvpppSegDataLayer<Dtype>::~CvpppSegDataLayer() {
  this->StopInternalThread();
}

template <typename Dtype>
void CvpppSegDataLayer<Dtype>::loadDatums(const std::string& filePath){
	datums_.clear();
...
template <typename Dtype>
void CvpppSegDataLayer<Dtype>::DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
...
template<typename Dtype>
void CvpppSegDataLayer<Dtype>::load_batch(Batch<Dtype>* batch) {
...
template<typename Dtype>
void CvpppSegDataLayer<Dtype>::TransformDataLabel(Blob<Dtype>* data_blob, Blob<Dtype>* label_blob){
...
//	if(dominantInstanceID!=datum.boxes[ranObjInd].instanceID){	
/*	if(true){	
...
template<typename Dtype>
CvpppDatum CvpppSegDataLayer<Dtype>::flipCvpppDatum(const CvpppDatum& datum, int mode){
...
template<typename Dtype>
void CvpppSegDataLayer<Dtype>::randomAssignDetected(CvpppDatum& datum,const cv::Mat& labelImg,cv::Mat& memImg){
...
template<typename Dtype>
int CvpppSegDataLayer<Dtype>::getDominantInstanceID(const cv::Mat& labelImg){
	int fre[200] = {};
	for(int y=0;y<labelImg.rows;y++){
		for(int x=0;x<labelImg.cols;x++){
			int aLabel = labelImg.at<uchar>(y,x);
			CHECK_LT(aLabel,200)<<"sorry I can't handle label bigger than 200";
			if(aLabel>0){
				fre[aLabel]++;
			}
		}
...
INSTANTIATE_CLASS(CvpppSegDataLayer);
REGISTER_LAYER_CLASS(CvpppSegData);
